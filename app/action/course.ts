...
export async function getCurrentCourse() {
  return asyncHandler(async () => {
    ...
    const [courseRes, levelsRes, progressRes, subscriptionRes] = await Promise.all([
      insforge.database
        .from("courses")
        .select("*")
      ...
      insforge.database
        .from("level_progress")
        .select("level_id, score, completed")
        .eq("user_id", userId),
      insforge.database
        .from("subscriptions")
    ...
    const isPro =
      sub?.plan === "pro" &&
      sub?.status === "active" &&
      new Date(sub?.current_period_end || 0) > new Date();

    const progressMap = new Map(
      (progressRes.data ?? []).map((p: any) => [p.level_id, p])
    );

    let prevLevelWasCompleted = true;
    const levelsWithProgress = (levelsRes.data ?? []).map((level: any) => {
      ...
      return {
        ...
        isProLocked,
        percentage: Math.min(
          Math.round(((progress?.score || 0) / TOTAL_EXERCISES) * 100),
          100
        ),
      };
    });
